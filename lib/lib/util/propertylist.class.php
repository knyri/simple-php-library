...
class ChangeTrackingPropertyList extends PropertyList{
	protected
		...
		$cleared= array(),
		// props in this list don't count torwards the overall hasChanges
		$ignored= array()
	;
	public function setIgnoredProps($ary){
		$this->ignored= $ary;
	}
	public function discardChanges(){
		$this->changes= array();
		$this->ignoredChanges= array();
		$this->cleared= array();
	}
	public function discardChange($key){
		...
		}else if (array_key_exists($key, $this->ignoredChanges)){
			unset($this->ignoredChanges[$key]);
		}
	}
	/**
	 * @deprecated use commitChanges()
	 */
	...
	public function commitChange($key){
		if(array_key_exists($key, $this->changes)){
			...
			$this->data[$key]= $this->ignoredChanges[$key];
			unset($this->ignoredChanges[$key]);
		}
	}
	...
	public function hasChanged($key){
		return array_key_exists($key, $this->changes) || array_key_exists($key, $this->cleared) || array_key_exists($key, $this->ignoredChanges);
	}
	public function getChanges(){
		...
		foreach($this->ignoredChanges as $k => $v){
			$changes['new'][$k]= $v;
			$changes['old'][$k]= array_key_exists($k, $this->data) ? $this->data[$k] : null;
		}
		return $changes;
	...
	public function set($k, $v){
		if(
			...
			unset($this->ignoredChanges[$k]);
			return;
		}
		if(array_search($k, $this->ignored) !== false){
			$this->ignoredChanges[$k]= $v;
		}else{
			$this->changes[$k]= $v;
		}
	...
	public function get($key, $default= null){
		...
		if(array_key_exists($key, $this->ignoredChanges)){
			return $this->ignoredChanges[$key];
		}
...
// 		unset($this->data[$k]);
	...
	protected function getFinal(){
		return array_diff_key(array_merge($this->data, $this->changes, $this->ignoredChanges), $this->cleared);
	}
	/**
	 * @param array $list $key=>$value list
	 */
